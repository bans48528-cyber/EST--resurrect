...
当前设备运行 `M0.99A`，官方 MicroPython `v1.29.0` 已经真实集成、启动并执行电脑上传的 Python 源码；统一 C 服务 tick、传感器/按键/电池只读 API，以及马达只读状态与安全停止 API 均已完成实机验证。
...
M0.95A 已完成正常执行、异常和硬超时，但 Python VM 运行时没有轮询 USB，电脑主动停止会写入超时。M0.96A 修复 USB 主动停止，M0.97A 建立统一 C 服务 tick，M0.98A 开放正式只读传感器、按键和电池 API，M0.99A 再开放马达只读状态与安全停止。因此后续开发和恢复应使用 M0.99A，不应回退到 M0.95A。
...
- `Sensor`、`buttons`、`battery` 和 `Motor` 状态读取已是正式 API；各传感器便捷类型类、模式切换和重置仍未开放。
- `Motor.stop()` 已开放逐端口自由滑行和主动刹车，但 RAM 脚本尚不能启动马达；现有 `motor_type()`、`stop_all()`、`drive_mix()` 仍属于早期最小验证接口。
...
1. 开放单马达功率、闭环速度和相对角度运行，并验证 Python 循环、强制 GC、显式停止和异常清理。
...
- 发布包：`firmware/releases/M0.99A/est_minimal_upgrade_app_M0.99A.upgrade.bin`
- manifest：`firmware/releases/M0.99A/est_minimal_upgrade_app_M0.99A.manifest.json`
- SHA-256：`b01b773280fa2b80512c6db2d24c3340d19bf143b67eff238c7af40a6ddf5ae0`
- 固件测试：78 项通过。
- HID 工具测试：112 项通过。
