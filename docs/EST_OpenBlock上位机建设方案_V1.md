...
- 蓝牙支持；
- 兼容所有历史 EST 固件。最低兼容基线保留为 M0.72A / 协议 1.5，当前完整功能目标为 M0.90A / 协议 1.13；新增能力必须按设备功能位启用，M0.90A 定量转向在实机验收前保持测试功能标记。
...
2. M0.72A / 协议 1.5 是最低兼容基线，M0.90A / 协议 1.13 是当前完整功能目标；
3. 实时模式先于上传模式；
