...
- 当前实机版本：`M0.18A`。
- 已验证完整闭环：`M0.17A` 接收 260 包升级文件，设备冷启动后由官方 Bootloader 搬运，`M0.18A` 成功启动并返回心跳。
- `M0.18A` 启动表现：约 2 秒红蓝交替，随后红灯熄灭、蓝灯慢闪。
- `M0.18A` 已恢复官方“最终 ACK 后拉低 PE2 关机，等待人工重新开机”的语义，并已完成实机验证。
...
| `firmware/releases` | 已实机验证的 M0.16A/M0.17A/M0.18A 升级包和清单 |
