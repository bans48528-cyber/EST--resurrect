...
//	.	
//[15]0 1 2 3 ... 179 
volatile u8 LCD_GRAM[180][16]={0};			//180*128
...
void LCD_UC1638C_GPIO_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin =  GPIO_Pin_2 ;													// SDA PD9->PG2
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;                                                    
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOG, &GPIO_InitStructure); 
		 
	GPIO_InitStructure.GPIO_Pin =  GPIO_Pin_15;													// RST PD8->PD15
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;                                                    
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOD, &GPIO_InitStructure);

	GPIO_InitStructure.GPIO_Pin =  GPIO_Pin_14;													// SCK PB13->PD14
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;                                                    
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOD, &GPIO_InitStructure);  
...
void initialize(void)
{
	Delay(1000); LCD_UC1638C_RST_1;
	Delay(400);  LCD_UC1638C_RST_0;
	Delay(3200); LCD_UC1638C_RST_1;
	Delay(8000);
...
void iic_start(u8 cd)				//1send IIC start condition
{
	...
	if(!cd)	LCD_UC1638C_SDA_0;																	//模式选择，control和data,control用于选择模式，data用于在该模式下收发数据
...
void set_uc1638c_Addr(u8 page,u8 column) 
{
	...
	iic_start(0);
	iic_send((page%256)|0x60); 					// 0-3位 设置SRAM读写访问页地址从第0页开始
	iic_send((page/256&0x03)|0x70);			// 4、5位
	iic_stop();
	
	iic_start(0);												// 使能对屏幕的操作
	iic_send(0xf9);  
	iic_stop();
...
//显示2个数字
//x,y :起点坐标	 
//len :数字的位数,当len==0时位数显示不限
//size:字体大小
//mode:模式	0,填充模式;1,叠加模式
//num:数值(0~4294967295);	 		  
void LCD_ShowNum(u8 x,u8 y,u32 num,u8 len,u8 size,u8 mode)
{         	
	u8 t,temp;
	u8 enshow=0;	
	int number;
...
char myfloat[8] = {0};
void LCD_ShowFloat(u8 x,u8 y,float decnum,u8 size,u8 mode)
{
//	sprintf(myfloat, "%0.*f", 2, decnum);
	sprintf(myfloat, "%g", decnum);
	
	LCD_ShowString(x, y,myfloat, size, mode);	
}
...
//*p:字符串起始地址 
//mode:0:白底黑字; 1:白字黑底
void LCD_ShowString(u8 x,u8 y,const char *p,u8 size,u8 mode)
{
	while((*p<='~')&&(*p>=' '))//判断是不是非法字符!
	{		
...
//返回：无
//================================================================
void LCD_Display_slider(u8 x0, u8 y0, u8 hight, u16 total, u16 now)
{
	u8 i,j;
	u8 x1,y1;
	u8 h1 = hight/total;
	...
	if(h1<4) h1=4;
	x1 = x0;
	y1 = y0+hight*now/total;
	for(j=0;j<h1;j++)
	{
		for(i=0;i<4;i++)
		{
			LCD_DrawPoint(x1+i,y1+j,1);
		}
	}	
}
...
u32 fupd_prog(u16 x,u16 y,u8 size,u32 fsize,u32 pos)
{
	...
	if(t!=prog)
	{
		...
		t=prog;
		if(t>100)t=100;
		LCD_ShowNum(x,y,t,3,size,0);//显示数值
//		LCD_refresh();
	}
	return 0;					    
} 
